...
import { ArrowRight, Search, Code2, Cpu, Layers } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div className="relative">
            ...
            <p className="text-slate-400 mt-4 text-lg max-w-xl leading-relaxed">
              Engage in high-frequency coding battles. Execute logic, optimize
              runtime, and dominate the leaderboard.
            </p>
          </div>
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full p-6 md:p-8">
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${status === "live" ? "bg-green-400 animate-pulse" : "bg-current"}`}
            ></span>
            {status}
          </div>
        </div>
        ...
        <div className="relative rounded-xl bg-slate-900/50 border border-slate-800 p-4 mb-6 group-hover:border-slate-700 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-4">
            <DataPoint
              label="START"
              value={new Date(contest.startTime).toLocaleDateString(undefined, {
                month: "short",
                day: "numeric",
                hour: "2-digit",
              })}
            ...
            <DataPoint
              label="END"
              value={new Date(contest.endTime).toLocaleDateString(undefined, {
                month: "short",
                day: "numeric",
                hour: "2-digit",
              })}
...
const TechIcon = ({
  ...
  <div
    className={`w-8 h-8 rounded-full border border-slate-800 bg-slate-900 flex items-center justify-center ${color} shadow-lg relative z-0 ring-2 ring-slate-950`}
  >
    {icon}
  </div>
);
...
const FilterButton = ({
  ...
  >
    {label}
    {count !== undefined && (
      <span className="ml-2 text-xs opacity-50 font-mono">{count}</span>
    )}
  </button>
);
